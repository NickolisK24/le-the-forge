...
interface V2PassiveResponse {
  ...
  data_source?: string;
  source_path?: string;
  total_passive_trees?: number;
  total_passive_nodes?: number;
  result_count?: number;
...
function buildUrl(limit: number, query: string, classId: string): string {
  ...
  return `/experimental/v2/passives?${params.toString()}`;
}

async function fetchV2Passives(limit: number, query: string, classId: string): Promise<V2PassiveResponse> {
  ...
  return json as V2PassiveResponse;
}

export default function V2PassivesDebugPage() {
  const [limit, setLimit] = useState(DEFAULT_LIMIT);
  const [query, setQuery] = useState("");
  const [classId, setClassId] = useState("");
  const [data, setData] = useState<V2PassiveResponse | null>(null);
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <header className="border-b border-[#2a3050] pb-4">
        ...
        <h1 className="mt-2 font-display text-2xl text-[#f5a623]">v2 passive tree inspection</h1>
        <p className="mt-2 max-w-3xl text-sm text-gray-400">This page reads v2 passive tree records for diagnostics and does not power planner behavior.</p>
      </header>
        ...
        <>
          ...
          <section className="overflow-hidden rounded border border-[#2a3050] bg-[#10152a]">
            <div className="border-b border-[#2a3050] p-4">
              ...
              <p className="mt-1 text-xs text-gray-500">Showing {records.length} trees.</p>
            </div>
                ...
                <tbody className="divide-y divide-[#2a3050]">
                  {records.map((record) => (
                    <tr key={record.canonical_id}>
                      ...
                      <td className="px-4 py-3 font-mono text-gray-300">{record.owner_class_id ?? "n/a"}</td>
                      <td className="px-4 py-3 font-mono text-gray-300">{record.node_ids.length}</td>
