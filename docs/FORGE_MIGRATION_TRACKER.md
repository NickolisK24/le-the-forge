...
- The controlled modifier inspection stack milestone is complete for diagnostics: stat/modifier reference audit, modifier resolution policy, controlled modifier resolver prototype, saved-vs-fresh comparison, unresolved modifier category triage, missing modifier reference mapping policy and validation, malformed tier/value shape policy and validation, and unsupported special behavior policy all exist as non-production, read-only planning/tooling. Current state is `diagnostic_only=true`, `production_safe=false`, `comparison_status=warning`, 6959 total modifier references, 5596 resolved inspection-only references, 115 unresolved references, 136 malformed references, and 1112 unsupported references. The unresolved triage classifies those blockers as missing reference mapping, malformed tier/value shape, and unsupported special behavior. The missing mapping validator reports `validation_status=warning`: all 115 missing mapping records preserve raw evidence, source identity, provenance, and warning metadata; all remain unresolved; no display-name-only or `subtype_id`-only mapping inference is present; saved-vs-fresh unresolved delta is 0. The malformed tier/value validator reports `validation_status=warning`: all 136 malformed records preserve raw min/max/order evidence, provenance, and warning metadata; 136 diagnostic normalized views are labeled `diagnostic_only_not_source_mutation`; 34 inverted negative ranges are surfaced; no raw evidence is missing. The unsupported special behavior policy requires 1112 structures to remain unsupported until source-backed semantic evidence exists, forbids guessing from prose/text/name/tag/category evidence, and forbids collapsing unsupported behavior into generic stat modifiers. Count deltas and warning category deltas are zero; deterministic output, provenance coverage, and affix `910` duplicate evidence all agree. No gameplay/crafting/build-math semantics are inferred.
...
17. Treat missing modifier reference mapping policy validation as complete: `backend/app/game_data/missing_modifier_reference_mapping_validator.py` and `backend/scripts/report_missing_modifier_reference_mapping.py` generate `docs/generated/missing_modifier_reference_mapping_report.md` and `.json`. Current status is `warning`; all 115 missing mapping records preserve raw evidence, source identity, provenance, and warning metadata; all remain unresolved; no name-only or `subtype_id`-only mapping inference is present; saved-vs-fresh unresolved delta is 0.
...
19. Treat unsupported special behavior policy as documented in `docs/migration/MODIFIER_RESOLUTION_POLICY.md`: the 1112 unsupported structures must remain unsupported until source-backed semantic evidence exists; they must not be guessed from text, labels, names, tags, or categories; they must not be collapsed into generic stat modifiers.
20. Keep production output unchanged.
21. Keep `production_safe=false`.
...
2. Next recommended architecture target: implement diagnostic validation for unsupported special behavior. This should preserve raw evidence, provenance, and warning metadata before any resolver output changes.
