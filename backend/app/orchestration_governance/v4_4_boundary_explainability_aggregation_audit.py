"""Audit for deterministic v4.4 boundary explainability aggregation."""

from __future__ import annotations

from dataclasses import is_dataclass, replace
from typing import Any, Iterable

from .v4_4_boundary_explainability_aggregation_hashing import (
    deterministic_boundary_explainability_aggregation_hash,
    hash_boundary_explainability_aggregation_identity,
    hash_boundary_explainability_aggregation_intelligence,
    hash_diagnostic_aggregation_identity,
    hash_diagnostic_aggregation_record,
    hash_explainability_aggregation_record,
    hash_explanation_coverage_summary,
    hash_explanation_trace,
    hash_lineage_aggregation_summary,
    hash_phase_evidence_summary,
    hash_provenance_aggregation_summary,
    hash_replay_rollback_aggregation_summary,
    hash_source_evidence_reference,
)
from .v4_4_boundary_explainability_aggregation_models import (
    EXPLAINABILITY_STATE_EXPLAINED,
    EXPLAINABILITY_STATE_PARTIALLY_EXPLAINED,
    EXPLAINABILITY_STATE_UNEXPLAINED,
    V4_4_BOUNDARY_EXPLAINABILITY_AGGREGATION_REPORT_SCHEMA_VERSION,
    V4_4_BOUNDARY_EXPLAINABILITY_AGGREGATION_STATUS_BLOCKED,
    V4_4_BOUNDARY_EXPLAINABILITY_AGGREGATION_STATUS_STABLE,
    BoundaryExplainabilityAggregationIntelligence,
    default_boundary_explainability_aggregation_intelligence,
)
from .v4_4_boundary_explainability_aggregation_serialization import (
    export_boundary_explainability_aggregation_intelligence,
    serialize_boundary_explainability_aggregation_intelligence,
)
from .v4_4_boundary_explainability_aggregation_visibility import (
    count_combined_explainability_states,
    diagnostic_aggregation_summaries,
    explainability_coverage_summaries,
    fail_visible_diagnostic_summaries,
    governance_safe_explanation_traces,
    lineage_visibility,
    provenance_visibility,
    source_evidence_coverage_totals,
    unresolved_diagnostic_totals,
    validate_required_explainability_visibility,
)


CAPABILITY_COUNTER_FIELD_MAP: dict[str, tuple[str, ...]] = {
    "enabled_runtime_execution_count": (
        "runtime_execution_enabled",
        "orchestration_runtime_behavior_enabled",
        "orchestration_execution_enabled",
        "execution_enabled",
    ),
    "enabled_orchestration_authorization_count": (
        "orchestration_authorization_enabled",
        "authorization_enabled",
    ),
    "enabled_orchestration_approval_count": (
        "orchestration_approval_enabled",
        "approval_enabled",
        "readiness_approval_enabled",
    ),
    "enabled_dispatch_execution_count": (
        "dispatch_execution_enabled",
        "dispatch_enabled",
        "orchestration_dispatch_enabled",
    ),
    "enabled_routing_execution_count": (
        "routing_execution_enabled",
        "routing_enabled",
        "orchestration_routing_enabled",
    ),
    "enabled_scheduling_execution_count": (
        "scheduling_execution_enabled",
        "scheduling_enabled",
        "orchestration_scheduling_enabled",
    ),
    "enabled_recommendation_count": (
        "recommendation_enabled",
        "orchestration_recommendation_enabled",
        "explainability_based_recommendation_enabled",
    ),
    "enabled_decision_count": (
        "decision_enabled",
        "orchestration_decision_enabled",
    ),
    "enabled_operational_mutation_count": (
        "operational_mutation_enabled",
        "runtime_mutation_enabled",
        "mutation_enabled",
    ),
}
PROHIBITED_BOOLEAN_FIELD_NAMES: tuple[str, ...] = tuple(
    sorted({field for fields in CAPABILITY_COUNTER_FIELD_MAP.values() for field in fields})
)


def build_v4_4_boundary_explainability_aggregation() -> BoundaryExplainabilityAggregationIntelligence:
    return default_boundary_explainability_aggregation_intelligence()


def _iter_dataclass_objects(value: Any) -> Iterable[Any]:
    if is_dataclass(value) and not isinstance(value, type):
        yield value
        for item in value.__dict__.values():
            yield from _iter_dataclass_objects(item)
    elif isinstance(value, dict):
        for item in value.values():
            yield from _iter_dataclass_objects(item)
    elif isinstance(value, tuple | list | set):
        for item in value:
            yield from _iter_dataclass_objects(item)


def enabled_explainability_aggregation_capability_flags(
    intelligence: BoundaryExplainabilityAggregationIntelligence,
) -> dict[str, list[str]]:
    enabled: dict[str, list[str]] = {}
    for item in _iter_dataclass_objects(intelligence):
        item_id = (
            getattr(item, "source_id", None)
            or getattr(item, "summary_id", None)
            or getattr(item, "explanation_id", None)
            or getattr(item, "diagnostic_id", None)
            or getattr(item, "coverage_id", None)
            or getattr(item, "trace_id", None)
            or getattr(item, "provenance_id", None)
            or getattr(item, "lineage_id", None)
            or getattr(item, "evidence_id", None)
            or getattr(item, "diagnostic_aggregation_id", None)
            or getattr(item, "aggregation_id", item.__class__.__name__)
        )
        for field_name in PROHIBITED_BOOLEAN_FIELD_NAMES:
            if bool(getattr(item, field_name, False)):
                enabled.setdefault(str(item_id), []).append(field_name)
    return {key: sorted(values) for key, values in sorted(enabled.items())}


def explainability_aggregation_capability_counter_values(
    intelligence: BoundaryExplainabilityAggregationIntelligence,
) -> dict[str, int]:
    objects = tuple(_iter_dataclass_objects(intelligence))
    counters: dict[str, int] = {}
    for counter_name, field_names in CAPABILITY_COUNTER_FIELD_MAP.items():
        counters[counter_name] = sum(
            1
            for item in objects
            for field_name in field_names
            if bool(getattr(item, field_name, False))
        )
    return counters


def boundary_explainability_aggregation_equal(
    left: BoundaryExplainabilityAggregationIntelligence,
    right: BoundaryExplainabilityAggregationIntelligence,
) -> bool:
    return serialize_boundary_explainability_aggregation_intelligence(left) == serialize_boundary_explainability_aggregation_intelligence(right)


def validate_explainability_aggregation_ordering_stability(
    intelligence: BoundaryExplainabilityAggregationIntelligence,
) -> dict[str, Any]:
    order_groups = {
        "source_evidence_references": tuple(
            item.deterministic_order for item in intelligence.source_evidence_references
        ),
        "phase_evidence_summaries": tuple(
            item.deterministic_order for item in intelligence.phase_evidence_summaries
        ),
        "explainability_records": tuple(
            item.deterministic_order for item in intelligence.explainability_records
        ),
        "diagnostic_records": tuple(item.deterministic_order for item in intelligence.diagnostic_records),
        "coverage_summaries": tuple(item.deterministic_order for item in intelligence.coverage_summaries),
        "explanation_traces": tuple(item.deterministic_order for item in intelligence.explanation_traces),
    }
    unordered_groups = [name for name, orders in order_groups.items() if tuple(sorted(orders)) != orders]
    duplicate_groups = [name for name, orders in order_groups.items() if len(set(orders)) != len(orders)]
    return {
        "valid": not unordered_groups and not duplicate_groups,
        "unordered_groups": unordered_groups,
        "duplicate_order_groups": duplicate_groups,
        "order_groups": {name: list(orders) for name, orders in order_groups.items()},
    }


def validate_explainability_aggregation_serialization_and_hashing(
    intelligence: BoundaryExplainabilityAggregationIntelligence,
) -> dict[str, Any]:
    rebuilt = build_v4_4_boundary_explainability_aggregation()
    serialized = serialize_boundary_explainability_aggregation_intelligence(intelligence)
    rebuilt_serialized = serialize_boundary_explainability_aggregation_intelligence(rebuilt)
    intelligence_hash = hash_boundary_explainability_aggregation_intelligence(intelligence)
    rebuilt_hash = hash_boundary_explainability_aggregation_intelligence(rebuilt)
    return {
        "valid": serialized == rebuilt_serialized and intelligence_hash == rebuilt_hash,
        "explainability_serialization_stable": serialized == rebuilt_serialized,
        "diagnostic_serialization_stable": serialized == rebuilt_serialized,
        "explainability_hashing_stable": intelligence_hash == rebuilt_hash,
        "diagnostic_hashing_stable": intelligence_hash == rebuilt_hash,
        "serialization_length": len(serialized),
        "intelligence_hash": intelligence_hash,
        "rebuilt_intelligence_hash": rebuilt_hash,
    }


def validate_explainability_aggregation_visibility(
    intelligence: BoundaryExplainabilityAggregationIntelligence,
) -> dict[str, Any]:
    required = validate_required_explainability_visibility(intelligence)
    source = source_evidence_coverage_totals(intelligence)
    coverage = explainability_coverage_summaries(intelligence)
    diagnostics = diagnostic_aggregation_summaries(intelligence)
    unresolved = unresolved_diagnostic_totals(intelligence)
    traces = governance_safe_explanation_traces(intelligence.explanation_traces)
    return {
        "valid": (
            required["valid"]
            and source["all_sources_available"]
            and source["production_consumption_enabled_count"] == 0
            and coverage["recommendation_enabled_count"] == 0
            and coverage["decision_enabled_count"] == 0
            and coverage["runtime_readiness_inferred_count"] == 0
            and diagnostics["diagnostic_auto_resolution_enabled_count"] == 0
            and diagnostics["automatic_remediation_enabled_count"] == 0
            and diagnostics["automatic_repair_enabled_count"] == 0
            and diagnostics["recommendation_enabled_count"] == 0
            and diagnostics["decision_enabled_count"] == 0
            and diagnostics["authorization_enabled_count"] == 0
            and unresolved["unresolved_diagnostic_count"] > 0
            and traces["recommendation_enabled_count"] == 0
            and traces["decision_enabled_count"] == 0
            and traces["runtime_readiness_inferred_count"] == 0
        ),
        "combined_counts": count_combined_explainability_states(intelligence),
        "missing_states": required["missing_states"],
        "missing_fail_visible_states": required["missing_fail_visible_states"],
        "source": source,
        "coverage": coverage,
        "diagnostics": diagnostics,
        "unresolved": unresolved,
        "traces": traces,
    }


def validate_explainability_replay_rollback_evidence(
    intelligence: BoundaryExplainabilityAggregationIntelligence,
) -> dict[str, Any]:
    evidence = intelligence.replay_rollback_summary
    expected = (
        set(evidence.source_evidence_ids)
        | set(evidence.explanation_record_ids)
        | set(evidence.diagnostic_record_ids)
    )
    replay = set(evidence.replay_evidence_ids)
    rollback = set(evidence.rollback_evidence_ids)
    return {
        "valid": evidence.replay_safe and evidence.rollback_safe and replay == expected and rollback == expected,
        "expected_evidence_count": len(expected),
        "replay_safe_evidence_count": len(replay),
        "rollback_safe_evidence_count": len(rollback),
        "missing_replay_evidence_ids": sorted(expected - replay),
        "missing_rollback_evidence_ids": sorted(expected - rollback),
        "replay_safe": evidence.replay_safe,
        "rollback_safe": evidence.rollback_safe,
    }


def validate_explainability_provenance_lineage_visibility(
    intelligence: BoundaryExplainabilityAggregationIntelligence,
) -> dict[str, Any]:
    provenance = intelligence.provenance_summary
    lineage = intelligence.lineage_summary
    return {
        "valid": (
            provenance.provenance_visible
            and lineage.lineage_visible
            and not provenance.hidden_source_inference_used
            and not lineage.ambiguous_lineage_inferred
        ),
        "provenance_visible": provenance.provenance_visible,
        "lineage_visible": lineage.lineage_visible,
        "hidden_source_inference_used": provenance.hidden_source_inference_used,
        "ambiguous_lineage_inferred": lineage.ambiguous_lineage_inferred,
    }


def validate_explainability_aggregation_non_operational(
    intelligence: BoundaryExplainabilityAggregationIntelligence,
) -> dict[str, Any]:
    counters = explainability_aggregation_capability_counter_values(intelligence)
    enabled_flags = enabled_explainability_aggregation_capability_flags(intelligence)
    root_disabled = {
        "descriptive_only": intelligence.descriptive_only,
        "non_operational": intelligence.non_operational,
        "non_authoritative": intelligence.non_authoritative,
        "non_recommending": intelligence.non_recommending,
        "non_deciding": intelligence.non_deciding,
        "non_remediating": intelligence.non_remediating,
        "non_mutating": intelligence.non_mutating,
        "runtime_readiness_inference_disabled": intelligence.runtime_readiness_inference_disabled,
        "planner_integration_disabled": not intelligence.planner_integration_enabled,
        "production_consumption_disabled": not intelligence.production_consumption_enabled,
        "runtime_mutation_disabled": not intelligence.runtime_mutation_enabled,
        "operational_mutation_disabled": not intelligence.operational_mutation_enabled,
        "diagnostic_auto_resolution_disabled": not intelligence.diagnostic_auto_resolution_enabled,
        "explainability_based_recommendation_disabled": (
            not intelligence.explainability_based_recommendation_enabled
        ),
        "automatic_remediation_disabled": not intelligence.automatic_remediation_enabled,
        "automatic_repair_disabled": not intelligence.automatic_repair_enabled,
    }
    return {
        "valid": all(value == 0 for value in counters.values()) and all(root_disabled.values()),
        "counters": counters,
        "enabled_flags": enabled_flags,
        **root_disabled,
    }


def validate_boundary_explainability_aggregation(
    intelligence: BoundaryExplainabilityAggregationIntelligence,
) -> dict[str, Any]:
    validations = {
        "ordering": validate_explainability_aggregation_ordering_stability(intelligence),
        "serialization_hashing": validate_explainability_aggregation_serialization_and_hashing(intelligence),
        "visibility": validate_explainability_aggregation_visibility(intelligence),
        "replay_rollback": validate_explainability_replay_rollback_evidence(intelligence),
        "provenance_lineage": validate_explainability_provenance_lineage_visibility(intelligence),
        "non_operational": validate_explainability_aggregation_non_operational(intelligence),
    }
    invalid = [name for name, result in validations.items() if not result["valid"]]
    return {
        "valid": not invalid,
        "invalid_validation_names": invalid,
        "validation_error_count": len(invalid),
        "validations": validations,
    }


def build_v4_4_boundary_explainability_aggregation_report() -> dict[str, Any]:
    intelligence = build_v4_4_boundary_explainability_aggregation()
    exported = export_boundary_explainability_aggregation_intelligence(intelligence)
    validation = validate_boundary_explainability_aggregation(intelligence)
    visibility = validation["validations"]["visibility"]
    serialization_hashing = validation["validations"]["serialization_hashing"]
    replay_rollback = validation["validations"]["replay_rollback"]
    provenance_lineage = validation["validations"]["provenance_lineage"]
    non_operational = validation["validations"]["non_operational"]
    coverage = visibility["coverage"]
    diagnostics = visibility["diagnostics"]
    unresolved = visibility["unresolved"]
    deterministic_hash_evidence = {
        "identity_hash": hash_boundary_explainability_aggregation_identity(intelligence.identity),
        "diagnostic_identity_hash": hash_diagnostic_aggregation_identity(
            intelligence.diagnostic_identity
        ),
        "intelligence_hash": hash_boundary_explainability_aggregation_intelligence(intelligence),
        "provenance_hash": hash_provenance_aggregation_summary(intelligence.provenance_summary),
        "lineage_hash": hash_lineage_aggregation_summary(intelligence.lineage_summary),
        "replay_rollback_hash": hash_replay_rollback_aggregation_summary(
            intelligence.replay_rollback_summary
        ),
        "source_evidence_hashes": {
            record.source_id: hash_source_evidence_reference(record)
            for record in sorted(
                intelligence.source_evidence_references,
                key=lambda item: item.deterministic_order,
            )
        },
        "phase_summary_hashes": {
            record.summary_id: hash_phase_evidence_summary(record)
            for record in sorted(
                intelligence.phase_evidence_summaries,
                key=lambda item: item.deterministic_order,
            )
        },
        "explainability_hashes": {
            record.explanation_id: hash_explainability_aggregation_record(record)
            for record in sorted(
                intelligence.explainability_records,
                key=lambda item: item.deterministic_order,
            )
        },
        "diagnostic_hashes": {
            record.diagnostic_id: hash_diagnostic_aggregation_record(record)
            for record in sorted(
                intelligence.diagnostic_records,
                key=lambda item: item.deterministic_order,
            )
        },
        "coverage_hashes": {
            record.coverage_id: hash_explanation_coverage_summary(record)
            for record in sorted(
                intelligence.coverage_summaries,
                key=lambda item: item.deterministic_order,
            )
        },
        "trace_hashes": {
            record.trace_id: hash_explanation_trace(record)
            for record in sorted(
                intelligence.explanation_traces,
                key=lambda item: item.deterministic_order,
            )
        },
    }
    summary = {
        "explainability_aggregation_record_count": len(intelligence.explainability_records),
        "diagnostic_aggregation_record_count": len(intelligence.diagnostic_records),
        "source_evidence_reference_count": len(intelligence.source_evidence_references),
        "phase_evidence_summary_count": len(intelligence.phase_evidence_summaries),
        "explanation_trace_count": len(intelligence.explanation_traces),
        "explained_count": coverage["explained_count"],
        "partially_explained_count": coverage["partially_explained_count"],
        "unexplained_count": coverage["unexplained_count"],
        "diagnostic_severity_totals": diagnostics["severity_counts"],
        "diagnostic_source_totals": diagnostics["diagnostic_source_counts"],
        "unresolved_diagnostic_count": unresolved["unresolved_diagnostic_count"],
        "fail_visible_diagnostic_count": diagnostics["fail_visible_diagnostic_count"],
        "deterministic_ordering_verified": validation["validations"]["ordering"]["valid"],
        "explainability_serialization_verified": serialization_hashing["explainability_serialization_stable"],
        "diagnostic_serialization_verified": serialization_hashing["diagnostic_serialization_stable"],
        "explainability_hashing_verified": serialization_hashing["explainability_hashing_stable"],
        "diagnostic_hashing_verified": serialization_hashing["diagnostic_hashing_stable"],
        "source_evidence_reference_stability_verified": visibility["source"]["all_sources_available"],
        "explainability_coverage_preserved": coverage["descriptive_only"],
        "diagnostic_visibility_preserved": diagnostics["descriptive_only"],
        "unresolved_diagnostic_preserved": unresolved["unresolved_diagnostic_count"] > 0,
        "replay_safe_evidence_verified": replay_rollback["replay_safe"],
        "rollback_safe_evidence_verified": replay_rollback["rollback_safe"],
        "provenance_visibility_verified": provenance_lineage["provenance_visible"],
        "lineage_visibility_verified": provenance_lineage["lineage_visible"],
        "governance_safe_certification_verified": validation["valid"],
        "non_operational_certification_verified": non_operational["valid"],
        "validation_error_count": validation["validation_error_count"],
        "remaining_warning_count": 0,
        "remaining_blocker_count": 0,
        **non_operational["counters"],
        "planner_integration_enabled": intelligence.planner_integration_enabled,
        "production_consumption_enabled": intelligence.production_consumption_enabled,
        "runtime_mutation_enabled": intelligence.runtime_mutation_enabled,
        "operational_mutation_enabled": intelligence.operational_mutation_enabled,
    }
    report: dict[str, Any] = {
        "report_schema_version": V4_4_BOUNDARY_EXPLAINABILITY_AGGREGATION_REPORT_SCHEMA_VERSION,
        "phase_id": intelligence.identity.phase_id,
        "foundation_status": (
            V4_4_BOUNDARY_EXPLAINABILITY_AGGREGATION_STATUS_STABLE
            if validation["valid"]
            else V4_4_BOUNDARY_EXPLAINABILITY_AGGREGATION_STATUS_BLOCKED
        ),
        "summary": summary,
        "boundary_explainability_aggregation_intelligence": exported,
        "source_evidence_coverage": visibility["source"],
        "explainability_coverage_visibility": coverage,
        "diagnostic_aggregation_visibility": diagnostics,
        "unresolved_diagnostic_visibility": unresolved,
        "fail_visible_diagnostic_visibility": fail_visible_diagnostic_summaries(intelligence),
        "provenance_visibility_evidence": provenance_visibility(intelligence),
        "lineage_visibility_evidence": lineage_visibility(intelligence),
        "governance_safe_explanation_traces": visibility["traces"],
        "deterministic_hash_evidence": deterministic_hash_evidence,
        "deterministic_serialization_evidence": {
            "explainability_serialization_stable": serialization_hashing["explainability_serialization_stable"],
            "diagnostic_serialization_stable": serialization_hashing["diagnostic_serialization_stable"],
            "explainability_hashing_stable": serialization_hashing["explainability_hashing_stable"],
            "diagnostic_hashing_stable": serialization_hashing["diagnostic_hashing_stable"],
            "serialization_length": serialization_hashing["serialization_length"],
            "intelligence_hash": serialization_hashing["intelligence_hash"],
        },
        "replay_safe_evidence": replay_rollback,
        "rollback_safe_evidence": replay_rollback,
        "non_operational_certification_evidence": non_operational,
        "validation": validation,
        "remaining_warnings": [],
        "remaining_blockers": [],
    }
    report["deterministic_report_hash"] = deterministic_boundary_explainability_aggregation_hash(
        report
    )
    return report


def contaminate_explainability_aggregation_for_non_operational_validation(
    intelligence: BoundaryExplainabilityAggregationIntelligence,
) -> BoundaryExplainabilityAggregationIntelligence:
    contaminated_explanation = replace(
        intelligence.explainability_records[0],
        recommendation_enabled=True,
        decision_enabled=True,
        runtime_readiness_inferred=True,
    )
    contaminated_diagnostic = replace(
        intelligence.diagnostic_records[0],
        diagnostic_auto_resolution_enabled=True,
        automatic_remediation_enabled=True,
        authorization_enabled=True,
    )
    return replace(
        intelligence,
        explainability_records=(contaminated_explanation,) + intelligence.explainability_records[1:],
        diagnostic_records=(contaminated_diagnostic,) + intelligence.diagnostic_records[1:],
        routing_execution_enabled=True,
        dispatch_execution_enabled=True,
        scheduling_execution_enabled=True,
        runtime_execution_enabled=True,
        operational_mutation_enabled=True,
    )
