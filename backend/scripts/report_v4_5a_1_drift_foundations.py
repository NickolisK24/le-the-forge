"""Generate deterministic v4.5A.1 drift foundation visibility."""
...
REPORT_PATH = Path("docs/generated/v4_5a_1_drift_foundations_report.json")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="v4.5A.1 drift foundations JSON report output path",
    )
...
def main() -> int:
    ...
    report = build_v4_5a_1_drift_foundations_report()
    write_report(report, output_path)
    summary = report["summary"]
    print(f"wrote {output_path}")
    print(f"foundation_status={report['foundation_status']}")
    print(f"drift_identity_count={summary['drift_identity_count']}")
    print(f"drift_classification_count={summary['drift_classification_count']}")
    print(f"evidence_reference_count={summary['evidence_reference_count']}")
    print(f"continuity_visibility_count={summary['continuity_visibility_count']}")
    print(f"diagnostic_count={summary['diagnostic_count']}")
    print(f"severity_visibility_count={summary['severity_visibility_count']}")
    print(
    ...
    print(
        "fail_visible_unsupported_state_visibility_verified="
        f"{summary['fail_visible_unsupported_state_visibility_verified']}"
    )
    print(
