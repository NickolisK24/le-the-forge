"""Generate deterministic v4.5A.3 integrity degradation intelligence visibility."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="v4.5A.3 integrity degradation intelligence JSON report output path",
    )
...
def main() -> int:
    ...
    report = build_v4_5a_3_integrity_degradation_intelligence_report()
    write_report(report, output_path)
    summary = report["summary"]
    print(f"wrote {output_path}")
    print(f"foundation_status={report['foundation_status']}")
    print(f"degradation_record_count={summary['degradation_record_count']}")
    print(f"degradation_classification_count={summary['degradation_classification_count']}")
    print(f"degradation_evidence_chain_count={summary['degradation_evidence_chain_count']}")
    print(f"continuity_degradation_count={summary['continuity_degradation_count']}")
    print(
        "degradation_severity_accumulation_count="
        f"{summary['degradation_severity_accumulation_count']}"
    )
    print(f"explainability_degradation_count={summary['explainability_degradation_count']}")
    print(f"cross_boundary_integrity_count={summary['cross_boundary_integrity_count']}")
    print(f"degradation_diagnostic_count={summary['degradation_diagnostic_count']}")
    print(
        "unsupported_degradation_state_count="
        f"{summary['unsupported_degradation_state_count']}"
    )
    print(
    ...
    print(f"integrity_continuity_preserved={summary['integrity_continuity_preserved']}")
    ...
    print(
        "degradation_explainability_verified="
        f"{summary['degradation_explainability_verified']}"
    )
    print(
        "fail_visible_unsupported_degradation_verified="
        f"{summary['fail_visible_unsupported_degradation_verified']}"
    )
    print(
    ...
    print(
        ...
        f"{summary['enabled_degradation_suppression_count']}"
    )
